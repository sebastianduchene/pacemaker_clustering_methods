...
def fit_GMM(brlen_matrix, n_comp, gene_names = None):
    
    fit_diag = mixture.GMM(n_components = n_comp, covariance_type = 'diag', n_iter = 10000).fit(brlen_matrix)
    pred_diag = fit_diag.predict(brlen_matrix)
    prob_diag = fit_diag.predict_proba(brlen_matrix)
    
    fit_sphe = mixture.GMM(n_components = n_comp, covariance_type = 'spherical', n_iter = 10000).fit(brlen_matrix)
...
print 'fitting mixture models using Dirichlet process prior with %s components' %(sys.argv[2])
...
mixture_models = fit_GMM(brlen_matrix, int(sys.argv[2]), gene_names)

mixture_models[0].to_csv('summary_GMM.csv')
...
print 'saved results to: brlen_clusters.pdf, summary_GMM.csv, prediction_diagonal.csv, probabilities_diagonal.csv, prediction_spherical.csv, probabilities_spherical.csv'
