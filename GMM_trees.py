...
def make_pam_sims(data, model, reps):
    sim_reps = []
    cluster_assignment = model.predict(data)
    cluster_freq = sc.itemfreq(cluster_assignment)

    if len(cluster_freq) == 1:
        for n in range(reps):
            data_sims = pd.DataFrame(np.zeros(shape = data.shape))
    ...
    elif len(cluster_freq) > 1:
        for n in range(reps):
            data_sims = pd.DataFrame(np.zeros(shape = data.shape))
            for clu in cluster_freq[:, 0]:
                cluster_loc = cluster_assignment == clu
            	dat_subset = data.ix[cluster_loc, ]
